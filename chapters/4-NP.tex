...
\z
%{\textcolor[rgb]{1,1,1} k} {\textcolor[rgb]{1,1,1} k}  \hspace{4mm}
\ea
\label{ex:4:30}
(LS-110714-1)\\
	...
	\begin{small}
		{\begin{tabular} {p{2.5cm} p{1cm} r r r} \midrule
			...
			Observed  & 0     &23   &8 &74.2\\
			 &1 &8 &52 &86.7\\
			 & & &Overall &82.0\\ \addlinespace[2mm]  \midrule
		\end{tabular}}
\end{small}
\caption{\textit{Model accuracy. Classification table for the model (1: switch at the phrase boundary; cut value = 0.50). (The table representation is based on   \citealt{bresnan-etal}.)}} \label{tab:4:11}
\end{table}
...
\begin{small}
		{\begin{tabularx}{\textwidth}{L{2.5cm}YYYYY}
		 \lsptoprule
			\addlinespace[2mm] 
            	\textit{Factor} &Estimate &Std. error &\textit{z}  &Pr(\textgreater\textbar{\textit{z}}\textbar) & \\ \addlinespace[2mm]  \midrule
			\addlinespace[2mm] 
			...
			\addlinespace[2mm] 
			\multicolumn{6}{p{\textwidth}}{\textit{Random effect:}}\\
			\multicolumn{6}{p{\textwidth}}{Speaker}\\
			\multicolumn{6}{p{\textwidth}}{(intercept, N = 17, variance = 0.999, σ = 0.999)}\\
			\addlinespace[2mm]
			\midrule 
			\addlinespace[2mm] 
			\multicolumn{6}{p{\textwidth}}{\textit{Summary statistics:}}\\
			\multicolumn{4}{l}{N} & & 91\\
			\multicolumn{4}{l}{\% correct predictions (\% baseline)} & & 82(65)\\
			\multicolumn{4}{l}{\textit{C} index of concordance} & & 82(65)\\
			\multicolumn{4}{l}{Somer's Dxy} & & 0.805\\
			\addlinespace[2mm] 
			\lspbottomrule
		\end{tabularx}}
\end{small}
}
\caption{\textit{Predicting switch placement in the context of the noun phrase modified by an adjective: minimal adequate generalised liner mixed model. Predicted odds are for switch placement outside the noun phrase. Significance codes: *significant at p \textless .05, **p \textless .01, ***p \textless .001.}} \label{tab:4:12} 
\end{table}
